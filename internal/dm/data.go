...
func ExportData(opts DataExportOptions) (*DataExportResult, error) {
	...
	assistByParentID := assistIndexesByParentID(tables, indexObjects, indexes, tablespaces)
...
func assistIndexesByParentID(tables map[uint32]dictionaryObject, indexObjects map[uint32]dictionaryObject, indexes map[uint32]indexDef, tablespaces map[uint32]string) map[uint32][]indexDef {
...
func resolveDataFiles(controlPath string, dataDir string) ([]dataFileRef, error) {
	...
	tablespaceNames := make(map[uint32]string)
