...
**v0.5.4 主题：Protected Page Reads & Auditable Residual Recovery**
...
- **磁盘字典**：生成可人工审查和修改的 `dmdul_dict/*.tsv`，再次启动后可直接
  `load dictionary;`，无需重复 bootstrap；分区键、分区顺序和完整二进制边界值也会落盘。
...
| v0.5.4 | 用户页原样读取、午夜 TIME、二级索引排除、分区 DDL、序列运行值恢复、字典原子重建与可审计孤儿页恢复 |
