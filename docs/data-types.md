...
本文记录 dmdul 对 DM8 常规表字段的离线 DDL 恢复、物理行解码以及 SQL、CSV、DMP
...
CSV 使用 UTF-8 文本输出。二进制字段写为不带 `0x` 的大写十六进制，NULL 写为空字段；
因此 CSV 本身不能区分 NULL 与数据库中的空字符串语义。
